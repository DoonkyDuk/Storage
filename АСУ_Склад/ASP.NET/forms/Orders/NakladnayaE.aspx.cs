...
namespace IIS.АСУ_Склад
{
    ...
    public partial class НакладнаяE : BaseEditForm<Накладная>
    {
        ...
        protected override void PreApplyToControls()
        {
            ...
            {
                var заказ = ctrlСотрудник.SelectedMasterPK;
                ctrlСотрудник.MasterViewName = Заказ.Views.ЗаказL.Name;

                var langdef = ExternalLangDef.LanguageDef;
                var lcs = LoadingCustomizationStruct.GetSimpleStruct(typeof(Строка_Заказа), Строка_Заказа.Views.Строка_ЗаказаE);
                lcs.LimitFunction = langdef.GetFunction(langdef.funcEQ,
                                                new VariableDef(langdef.GuidType, Information.ExtractPropertyPath<Строка_Заказа>(x => x.Заказ)), заказ);
                var строкиЗаказа = DataServiceProvider.DataService.LoadObjects(lcs);

                foreach (var s in строкиЗаказа)
                {
                    var строкаЗаказа = (Строка_Заказа)s;
                    DataObject.Запись_в_Накладной.Add(new Запись_в_Накладной { Количество = строкаЗаказа.Количество, Товар = строкаЗаказа.Товар });
                }
            }
        }
